...
const processISO8601DateTime = (selfDescription, iso8601Match, localTimezone) => {
  if (iso8601Match[5] !== null) {
    throw new Error(`${selfDescription} does not support week of year style dates.`)
  }
  else if (iso8601Match[7] !== null) {
    throw new Error(`${selfDescription} does not support ordinal/Julian style dates.`)
  }
  ...
  const hour = isEOD === true ? 24 : parseInt(iso8601Match[10])
  ...
  if (iso8601Match[11] === undefined && iso8601Match[13] === undefined && iso8601Match[15] === undefined) {
    ...
    const realSecs = 60 * fracMinutes
    seconds = Math.trunc(realSecs)
    fracSeconds = realSecs % 1
  }
  else {
    ...
    if (iso8601Match[14] === undefined && iso8601Match[15] === undefined) {
      ...
      seconds = Match.trunc(realSeconds)
      fracSeconds = realSeconds % 1
    }
    else {
